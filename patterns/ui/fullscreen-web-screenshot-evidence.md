...
Единый способ получать воспроизводимые QA-доказательства в развёрнутом Chrome и не смешивать три
разных результата: видимую область окна, всю прокручиваемую страницу и фрагмент элемента.
...
Основной инструмент — Playwright + Chrome в headed-режиме с отдельным persistent profile. Он уже
используется в `tools/ui-evidence/`, умеет `viewport`, `fullPage`, `clip`, скрытие caret, остановку
анимаций, маскирование и временный stylesheet. Один общий модуль захвата должен обслуживать все
проекты; сценарий заказа описывает только действия, селекторы и ожидаемое состояние.
...
1. Открыть отдельный профиль Chrome в headed-режиме. Зафиксировать версию Chrome, Playwright,
   Windows, locale, `devicePixelRatio`, URL и размер visual viewport.
2. Максимизировать окно через CDP и проверить полученное состояние. Для воспроизводимой регрессии
   вместо физического окна разрешён фиксированный viewport, но Playwright предупреждает, что
...
9. Записать рядом manifest JSON и построить contact sheet. Автоматический gate проверяет PNG,
   размер, соответствие `innerWidth × innerHeight` выбранному scale, URL, наличие целевого элемента,
   отсутствие полностью пустого кадра, одинаковые размеры в прогоне и крупные внутренние белые
...
- Прогон сначала живёт в `.evidence-runs/`; `evidence/` меняется только после quality gate,
  просмотра contact sheet и явного `-Approve` с резервной копией предыдущих кадров.

## Как поймать ошибку

Кадр не проходит gate, если: PNG не читается; размер не совпадает с manifest; в одном прогоне
появились разные dimensions; URL не из разрешённого origin; target отсутствует, обрезан или перекрыт;
...
записано в manifest. После gate обязателен contact sheet всех Case ID.
